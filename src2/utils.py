...
class utils:
	...
	def calcdates(self,yt,mt=0,yf=0,mf=0):
		if mt == 0:
			mt = 12
		if mf == 0:
			mf = 1
	...
	def ymtodatetime(self,datelist,fixdate=27):
		result = []
		ty = int(date.today().year)
		tm = int(date.today().month)
		td = int(date.today().day)
		for d in datelist:
			yyyy = int(d[0:4])
			mm = int(d[4:6])
			if not d[6:8] == '':
				...
				if yyyy == ty and mm == tm:
					dd = td
				else:
					dd = fixdate
			result.append(date(yyyy,mm,dd))
		return result
		...
		for i,d in enumerate(datelist):
			...
			if not d[6:8] == '':
				...
				if yyyy == ty and mm == tm:
					...
					dd = fixdate
			result.append(date(yy,mm,dd))
		return result

	def transposelist(self,list0):
		if len(list0) < 1:
			return [[]]
		if len(list0[0]) < 1:
			return [[]]
